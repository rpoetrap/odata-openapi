...
module.exports.csdl2openapi = function (
  ...
  function schemasForStructuredType(schemas, qualifier, name, type, suffix) {
    ...
    for (let [name, property] of model.propertiesOfStructuredType(type)) {
      ...
      if ([SUFFIX.read, SUFFIX.create].includes(suffix)) schemaProperties[name] = schema(property);
      if (property.$Kind === "NavigationProperty") {
        if (property.$Collection && [SUFFIX.read, SUFFIX.create].includes(suffix)) {
          if (csdl.$Version === "2.0") {
            if (!omitNav) {
